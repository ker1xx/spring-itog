...
class BouquetTagController(private val bouquetTagService: BouquetTagService) {
    ...
    fun getTagsByBouquetId(@PathVariable bouquetId: Int): List<BouquetTagEntity>? {
        return bouquetTagService.getTagsByBouquetId(bouquetId)
    }

    // Получение всех BouquetEntity по TagId
    @GetMapping("/tag/{tagId}")
    fun getBouquetsByTagId(@PathVariable tagId: Int): List<BouquetTagEntity>? {
        return bouquetTagService.getBouquetByTagId(tagId)
    }
}
